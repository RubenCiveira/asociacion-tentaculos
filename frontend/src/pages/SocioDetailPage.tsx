    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-800">{socio.nombre} {socio.apellidos}</h2>
            <div className="flex items-center gap-2 mt-1">
              <Badge label={socio.activo ? 'Activo' : 'Baja'} variant={socio.activo ? 'green' : 'gray'} dot />
              <span className="text-sm text-gray-400">{edad} años</span>
            </div>
          </div>
        </div>
        <button
          onClick={() => setEditing(true)}
          className="flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"
        >
          <Pencil size={14} />
          Editar
        </button>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5">
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-4">Datos del socio</h3>
        <dl className="grid grid-cols-2 gap-4">
          ...
          {socio.notas && (
            <div className="col-span-2">
              ...
              <dd className="text-sm text-gray-800 mt-0.5 whitespace-pre-line">{socio.notas}</dd>
            </div>
          )}
        </dl>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-gray-400">{materiales.length} ítem{materiales.length !== 1 ? 's' : ''}</span>
        </div>
        {materiales.length === 0 ? (
            ...
            {materiales.map(m => (
              <li key={m.$id} className="flex items-center justify-between py-2.5">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{TIPO_MATERIAL_LABELS[m.tipo]}</p>
                </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-gray-400">{eventosDelSocio.length} evento{eventosDelSocio.length !== 1 ? 's' : ''}</span>
        </div>
