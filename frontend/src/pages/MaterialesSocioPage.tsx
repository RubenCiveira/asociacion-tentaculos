...
export default function MaterialesSocioPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Tipo',
      render: (m: MaterialSocio) => (
        <span className="text-sm text-gray-600">{TIPO_MATERIAL_LABELS[m.tipo]}</span>
      ),
    },
    {
      key: 'socio',
      header: 'Propietario',
      render: (m: MaterialSocio) => (
        <span className="text-sm text-gray-600">{socioMap.get(m.socio_id) ?? '—'}</span>
      ),
    },
    {
      key: 'prestado',
  ...
  return (
    <div className="max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Materiales de socios</h2>
