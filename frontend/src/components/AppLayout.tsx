...
export default function AppLayout() {
  ...
  const sidebar = (
    <nav className="flex flex-col h-full">
      <div className="px-4 pt-6 pb-4">
        <h1 className="text-xl font-bold text-brand-700">Tentáculos</h1>
        <p className="text-xs text-gray-400 mt-0.5">Gestión interna</p>
      </div>
      ...
      <div className="p-3 border-t border-gray-100 mt-auto">
        <div className="flex items-center gap-2 px-3 py-2 mb-1">
          ...
          <span className="text-xs text-gray-600 truncate">{user?.name ?? user?.email}</span>
        </div>
        <button
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar desktop */}
      <aside className="hidden md:flex w-56 bg-white border-r border-gray-100 flex-shrink-0 flex-col">
        {sidebar}
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="md:hidden bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3">
          ...
          <span className="font-semibold text-brand-700">Tentáculos</span>
        </header>
